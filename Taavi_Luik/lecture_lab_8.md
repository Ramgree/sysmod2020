...
- How does Event Storming relate to the UML?
  - It's a diagram type, but it's simpler than many UML diagrams. It still has notation, but it's mostly about colors. It has structure, but it's mostly about a single axis (time). So it's simple and intended to be exploratory.

- What is the goal of Event Storming in terms of software development and understanding?
  - To make sense of the domain complexity. There is little need to write software faster nowadays, but rather to write better software. And understanding the domain is a big part of it. The goal is to model an entire business line.
...
Once the basic steps of creating a diagram are known (both theoretical - what is the notation - and practical - how to draw it using the software I am using) then it becomes much more enjoyable to design and use diagrams. It's also interesting how the argument against a diagram type because of the complexity of notation becomes much more weak. Similarly with finding situations where a diagram type really fits in - the argument against the diagram because 'it's too niche' again almost disappears.
